// ============================================================================
// CONCORDİA CELES HOTEL - ADMIN PANEL JAVASCRIPT (GENEL ORTALAMA DÜZELTMELİ)
// ============================================================================
console.log('🚀 admin.js yüklendi (Genel Ortalama Sabitlendi)');
...
const SCALE = 20; // 1-5 puanı 100% e çevirir
...
function processData(data) {
    ...
    data.forEach(row => {
        // 1. GENEL ORTALAMA HESABI (Daha Esnek & Garantili)
        let rowVals = [];
        for (let key in row) {
            let v = parseFloat(row[key]);
            if (!isNaN(v) && v > 0) {
                // 1-5 arasıysa 100'e çevir, zaten 100'deyse olduğu gibi al
                rowVals.push(v <= 5 ? v * SCALE : v);
            }
        }
        if (rowVals.length > 0) {
        ...
        for (let dept in DEPT_MAP) {
            ...
            cols.forEach(col => {
                ...
                if (!isNaN(v) && v > 0) dVals.push(v <= 5 ? v * SCALE : v);
            });
            if (dVals.length > 0) {
    ...
    // TOPLU HESAPLAMALAR
    let generalAvg = 0;
    if (totalRowAverages.length > 0) {
        generalAvg = Math.round(totalRowAverages.reduce((a,b)=>a+b,0) / totalRowAverages.length);
    } else if (Object.keys(deptScores).length > 0) {
        // Fallback: Satır ortalaması yoksa departman ortalamalarının ortalamasını al
        let deptAvgs = Object.values(deptScores).map(arr => arr.reduce((a,b)=>a+b,0)/arr.length);
        generalAvg = Math.round(deptAvgs.reduce((a,b)=>a+b,0) / deptAvgs.length);
    }
    ...
    console.log('📊 Genel Ortalama Hesaplandı:', generalAvg);
...
function renderDashboard(stats) {
    ...
    if (tbody) {
        ...
        if (stats.rawData.length > 0) {
            ...
            last10.forEach(r => {
                let score = 0, count = 0;
                for(let k in r) { let v=parseFloat(r[k]); if(!isNaN(v)&&v>0){score+=(v<=5?v*SCALE:v); count++;} }
    ...
    console.log('✅ PANEL HAZIR! Ortalama:', stats.generalAvg, '| Departman:', stats.deptStats.length, '| Personel:', stats.staffStats.length);
}
